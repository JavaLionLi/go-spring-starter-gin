...
func requestIDMiddleware() startergin.Middleware {
	return startergin.NewMiddleware(100, func(c *gin.Context) {
...
func userRoutes(handler *userHandler) routekit.Registrar {
	return routekit.NewRegistrar(200, func(engine *gin.Engine, kit routekit.Kit) {
		group := engine.Group("/api/users", kit.Handler("login"))
		group.GET("/:id", handler.Profile)
		group.POST("", handler.Create)
		group.DELETE("/:id", kit.Handler("admin"), func(c *gin.Context) {
			c.Status(http.StatusNoContent)
		})
	})
}

func requireHeader(name string, value string) gin.HandlerFunc {
